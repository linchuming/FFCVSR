...
if __name__ == '__main__':
    ...
    output_dir = 'results/VID4/FFCVSR_motion'
    datasets = ['calendar', 'city', 'walk', 'foliage']

    model_path = 'model_ckpt/ffcvsr-motion.ckpt'
    update_T = 50
    ...
    for dataset in datasets:
        ...
        with tf.variable_scope('video_sr'):
            m = model()
            clips_x = m.extract_feature(lr)
            motions = m.flow_net(clips_x)
            aligned_x = m.align_feature(clips_x, motions)
            local_sr, local_feat = m.local_net(aligned_x, bic)
            local_sr = tf.clip_by_value(local_sr, 0, 1)
            refine_sr, refine_feat = m.refine_net(tf_pre_sr, tf_pre_feat, local_sr, local_feat, motions)
        ...
        with tf.Session() as sess:
            ...
            for i in range(1, len(lr_imgs) - t + 2):
                ...
                if i == 1:
                    ...
                    out = sess.run([local_sr, refine_sr, assign_refine_to_pre],
                                   feed_dict={lr: concat_lr, bic: concat_bic})
                start = time.time()
                if i == 1:
        ...
        avg_bic_ssim = np.mean(avg_bic_ssim[2:-2])
